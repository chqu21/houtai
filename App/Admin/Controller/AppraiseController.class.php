...
class AppraiseController extends CommonController {
    ...
    public function appraiseList($page = 1, $rows = 10,$search = array(),$sort = 'comments_id', $order = 'desc'){
        if(IS_POST){
            ...
            $limit = $limit. "," . $rows;
            $order = $sort.' '.$order;
            $column = "`comments_id`,`type_id`,`type`,`comments`,`score`,`member_id`,`member_name`,`class_time`,`teacher_id`,`comment_type`,`order_code`";
            ...
            $datagrid = array(
                ...
                'fields' => array(
                    '评价类型ID'    => array('field'=>'type_id','width'=>15,'sortable'=>true),
                    '评价类型'      => array('field'=>'type','width'=>15,'sortable'=>true),
                    ...
                    '操作'    => array('field'=>'comments_id','width'=>60,'sortable'=>true,'formatter'=>'adminAppraiseListOperateFormatter'),
                )
            );
