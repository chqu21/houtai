...
function adminSubjectSearch(that){
    var queryParams = $('#forum_appraiselist_datagrid').datagrid('options').queryParams;
    $.each($(that).parent('form').serializeArray(), function() {
            queryParams[this['name']] = this['value'];
    });

    $('#forum_appraiselist_datagrid').datagrid('reload');
}
...
function adminAddAppraise(id){
	...
	var url = '<?php echo U('forum/appraiseAdd');?>';
...
function adminSubjectEdit(id){
    ...
    var url = '<?php echo U('forum/appraiseEdit');?>';
    url += url.indexOf('?') != -1 ? '&id='+id : '?id='+id;
...
function adminMemberDelete(id){
	...
	$.messager.confirm('提示信息', '确定要删除吗？', function(result){
		...
		$.post('<?php echo U('Forum/appraiseDelete');?>', {id: id}, function(res){
			if(!res.status){
