...
function adminMemberAdd(){
	$('#admin_appraiselist_add_dialog').dialog({href:'<?php echo U('Student/studentAdd');?>'});
	$('#admin_appraiselist_add_dialog').dialog('open');
}
//编辑
...
function adminHeadPhotoEdit(id){
    ...
    url += url.indexOf('?') != -1 ? '&id='+id : '?id='+id;
    $('#admin_appraiselist_edit_photo_dialog').dialog({href:url});
    $('#admin_appraiselist_edit_photo_dialog').dialog('open');
}
