...
<!-- 添加学生 -->
<div id="course_groupcourse_add_dialog" class="easyui-dialog" title="添加学生" data-options="modal:true,closed:true,iconCls:'icons-application-application_add',buttons:[{text:'确定',iconCls:'icons-other-tick',handler:function(){$('#course_groupcourse_add_dialog_form').submit();}},{text:'取消',iconCls:'icons-arrow-cross',handler:function(){$('#course_groupcourse_add_dialog').dialog('close');}}]" style="width:750px;height:550px;"></div>

<!-- 编辑课程 -->
<div id="course_groupcourse_edit_dialog" class="easyui-dialog" title="编辑学生" data-options="modal:true,closed:true,iconCls:'icons-application-application_edit',buttons:[{text:'确定',iconCls:'icons-other-tick',handler:function(){$('#course_groupcourse_edit_dialog_form').submit();}},{text:'取消',iconCls:'icons-arrow-cross',handler:function(){$('#course_groupcourse_edit_dialog').dialog('close');}}]" style="width:750px;height:550px;"></div>

<script type="text/javascript">

    //搜索
    function adminStudentSearch(that){
        var queryParams = $('#course_groupcourse_datagrid').datagrid('options').queryParams;
        $.each($(that).parent('form').serializeArray(), function() {
            queryParams[this['name']] = this['value'];
        });

        $('#course_groupcourse_datagrid').datagrid('reload');
    }
    ...
    function adminMemberAdd(){
        $('#course_groupcourse_add_dialog').dialog({href:'<?php echo U('Course/groupCourseAdd');?>'});
    $('#course_groupcourse_add_dialog').dialog('open');
    }
    //编辑
    function adminMemberEdit(id){
        ...
        var url = '<?php echo U('Course/groupCourseEdit');?>';
        url += url.indexOf('?') != -1 ? '&id='+id : '?id='+id;
        $('#course_groupcourse_edit_dialog').dialog({href:url});
        $('#course_groupcourse_edit_dialog').dialog('open');
    }
    ...
    function adminMemberDelete(id){
        ...
        $.messager.confirm('提示信息', '确定要删除吗？', function(result){
            ...
            $.post('<?php echo U('Course/groupCourseDelete');?>', {id: id}, function(res){
                if(!res.status){
